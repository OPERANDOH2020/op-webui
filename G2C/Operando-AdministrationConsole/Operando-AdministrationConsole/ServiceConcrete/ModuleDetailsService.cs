...
namespace Operando_AdministrationConsole.ServiceConcrete
{
    ...
    {
        #region configuration information constant strings
        private const string AapiConfigurationInformation = "AAPI:"
        ...
        private const string OapiConfigurationInformation = "There is a config.properties file \\op-interfaces\\op-interfaces-oapi\\src\\main\\resources\\config.properties and a pom file \\op-interfaces\\op-interfaces-oapi\\pom.xml." +
        ...
        private const string RapiConfigurationInformation = "There is a config.properties file \\op-interfaces\\op-interfaces-rapi\\src\\main\\resources\\config.properties and a pom file \\op-interfaces\\op-interfaces-rapi\\pom.xml." +
                   " The config.properties file is populated from the pom, so you can read configuration from the properties file but you need to write to the pom to make lasting changes.";

        private const string PcConfigurationInformation = "op-core-pc/pc-server/src/main/resources/config/operando.properties";

        private const string OseConfigurationInformation = "op-core-ose/ose-server/src/main/resources/operando.properties";

        private const string RmConfigurationInformation = "Rights Management has a configuration file which has the following information user/password, the credentials for CAS. Other parameters that are now hardcoded are the following:"
                + "\n__DAN_url"
                + "\n__aapi_st_url"
                + "\n__aapi_tgt_url"
                + "\n__aapi_tckt_val_url"
        ...
        private const string BdaConfigurationInformation = "The configuration files are placed under /WEB-INF/classes/";

        private const string PdbConfigurationInformation = "op-core-pdb/pdb-server/src/main/resources/config/service.properties, op-core-pdb/pdb-server/src/main/resources/config/db.properties";

        private const string AeConfigurationDetails = "/WEB-INF/classes/db.properties";

        private const string LdbConfigurationInformation = "LogDB is divided in two submodules: LogDB, LogDBSearch." +
        ...
        private const string GkConfigurationInformation = "There is a config.properties file \\op-pdr\\op-pdr-gatekeeper\\src\\main\\resources\\config.properties and a pom file \\op-pdr\\op-pdr-gatekeeper\\pom.xml." +
                   " The config.properties file is populated from the pom, so you can read configuration from the properties file but you need to write to the pom to make lasting changes.";

        private const string DanConfigurationInformation = "Basic Configuration:"
        ...
        private const string RpmConfigurationInformation = "Basic Configuration: https://github.com/OPERANDOH2020/op-pdr/blob/master/op-pdr-rpm/eu.operando.pdr.rpm.server/src/main/resources/META-INF/persistence.xml";

        private const string AmcConfigurationInformation = "All available configuration can be found in the Web.config file of the Operando-AdministationConsole project." +
                   "\n" +
                   "\nConfiguration options include database connection strings and URLs for other modules.";

        private const string RgConfigurationInformation = "Report Generator module (inside OPERANDO UI):"
        ...
        private IDictionary<string, string> GetModuleConfigurationDetailsDictionaryIndexedByAcronym()
        {
            ...
            {
                {"AAPI", AapiConfigurationInformation},
                {"OAPI", OapiConfigurationInformation},
                {"RAPI", RapiConfigurationInformation},
                {"PC", PcConfigurationInformation},
                {"OSE", OseConfigurationInformation},
                {"RM", RmConfigurationInformation},
                {"BDA", BdaConfigurationInformation},
                {"AE", AeConfigurationDetails},
                {"PDB", PdbConfigurationInformation},
                {"LDB", LdbConfigurationInformation},
                {"GK", GkConfigurationInformation},
                {"DAN", DanConfigurationInformation},
                {"RPM", RpmConfigurationInformation},
                {"AMC", AmcConfigurationInformation},
                {"RG", RgConfigurationInformation}
            };
