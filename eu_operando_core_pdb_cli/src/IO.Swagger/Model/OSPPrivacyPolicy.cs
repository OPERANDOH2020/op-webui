...
namespace eu.operando.core.pdb.cli.Model
{
    ...
    public partial class OSPPrivacyPolicy :  IEquatable<OSPPrivacyPolicy>
    {
        ...
        public Dictionary<String, List<List<object>>> sortAP()
        {
            Dictionary<String, List<List<object>>> sortedAP = new Dictionary<string, List<List<object>>>();
            foreach(var ap in this.Policies)
            {
                List<object> line = new List<object>();
                line.Add(ap.Permission);
                line.Add(ap.Resource);
                line.Add(ap.Action);

                if (!sortedAP.ContainsKey(ap.Subject))
                {
                    sortedAP.Add(ap.Subject, new List<List<object>>());
                }

                sortedAP[ap.Subject].Add(line);
            }
